...
#=
file = content.urls[1]; blake2b_256 = sha256 = file.digests.sha256; md5 = file.digests.md5;PYPI_FILE = content.urls[1].url; run(`pypi-attestations verify pypi
...
function registry_info(resp)
  regst_info = JSON3.read(resp.body)
  return regst_info
end
...
")
  open("$file_name.md", "a") do io
    ...
    date = getdate()
    println(io, "# <=======CEAZ Build Integrity Triage=======>\n\n## Package: $pkg  | $date\n")
  end
end
...
function ownership_check(regst_info, file_name)
  ...
  write_mkd("Maintainer: ", maintainer_email, file_name)
end
